...
export const getTransactionHistory = async (req, res) => {
    try {
        ...
        if (type === 'deposits' || type === 'all') {
            transactions = transactions.concat(wallet.depositHistory.slice(0, Number(limit)).map(t => ({
                ...t,
                type: 'deposit',
            })));
        }
        if (type === 'withdrawals' || type === 'all') {
            transactions = transactions.concat(wallet.withdrawalHistory.slice(0, Number(limit)).map(t => ({
                ...t,
                type: 'withdrawal',
            })));
        }
